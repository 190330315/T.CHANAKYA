...
public class playActivity extends AppCompatActivity {
    ...
    class lotto extends Thread
    {
        ...
        void random(int []a)
        {
            ...
            h.put("host", host);
            h.put("number",String.valueOf(a[j]));
            h.put("history",p.getText().toString());
            final int in=a[j];
            ref.child(id).setValue(h).addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
                public void onSuccess(Void aVoid) {
                    speak(String.valueOf(in));
